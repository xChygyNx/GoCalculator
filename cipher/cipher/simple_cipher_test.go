...
func TestVigenere(t *testing.T) {
	for _, test := range vtests {
		v, err := NewVigenere(test.key)
		if err != nil {
			t.Fatalf("NewVigenere(%q) returned nil error, want not-nil error",
				test.key)
		}
	...
	for _, k := range []string{"", "a", "aa", "no way", "CAT", "3", "and,"} {
		if _, err := NewVigenere(k); err == nil {
			t.Fatalf("NewVigenere(%q) not returned error, "+
				"Want error return for invalid argument.", k)
		}
	}
}
